...
Adding a field to `SettingsRepository` (`shared/src/commonMain/kotlin/org/carlospinan/bloqueador/app/settings/SettingsRepository.kt`) is an interface change — every implementer and every hand-written fake must be updated in the same pass or the build breaks. This project uses hand-written fakes (no MockK), so there is no shortcut.
...
- Test fakes (hand-written, in `commonTest`/`androidUnitTest`): `settings/SettingsRepositoryTest.kt`, `home/HomeViewModelTest.kt`, `rules/domain/EvaluateIncomingCallUseCaseTest.kt`, `onboarding/DialerOnboardingViewModelTest.kt`, `onboarding/DialerOnboardingScreenTest.kt`
