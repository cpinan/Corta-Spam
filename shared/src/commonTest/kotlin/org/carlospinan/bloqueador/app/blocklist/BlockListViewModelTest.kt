...
class BlockListViewModelTest {
    ...
    private class FakeRuleRepository : RuleRepository {
        ...
        override suspend fun recordCallAttempt(number: String, timestampMillis: Long) {}
        override suspend fun countRecentAttempts(number: String, sinceTimestampMillis: Long) = 0
        override suspend fun deleteExpiredAttempts(beforeTimestampMillis: Long) {}
        override suspend fun addBlockedNumber(number: String, label: String?) {}
        override suspend fun removeBlockedNumber(id: Long) {}
        override suspend fun addAllowlistedNumber(number: String, label: String?) {}
        override suspend fun removeAllowlistedNumber(id: Long) {}
        override suspend fun addPatternRule(pattern: String, label: String?) {}
        override suspend fun togglePatternRule(id: Long, enabled: Boolean) {}
        override suspend fun removePatternRule(id: Long) {}
        override suspend fun addCountryRule(countryCode: String, countryName: String) {}
        override suspend fun toggleCountryRule(id: Long, enabled: Boolean) {}
        override suspend fun removeCountryRule(id: Long) {}
        override suspend fun addActionRule(label: String?, attempts: Int, windowMinutes: Int, patternId: Long?) {}
        override suspend fun toggleActionRule(id: Long, enabled: Boolean) {}
        override suspend fun removeActionRule(id: Long) {}
        override suspend fun addScheduleRule(label: String?, startMinute: Int, endMinute: Int) {}
        override suspend fun toggleScheduleRule(id: Long, enabled: Boolean) {}
    ...
    @Test
    fun `blockedNumbers reflects repository`() = runTest {
    ...
    @Test
    fun `allowlistedNumbers reflects repository`() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        val repo = FakeRuleRepository()
    ...
    @Test
    fun `patternRules reflects repository`() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        val repo = FakeRuleRepository()
    ...
    @Test
    fun `countryRules reflects repository`() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        val repo = FakeRuleRepository()
    ...
    @Test
    fun `counts update with flow changes`() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        val repo = FakeRuleRepository()
        val vm = BlockListViewModel(repo)
        advanceUntilIdle()
